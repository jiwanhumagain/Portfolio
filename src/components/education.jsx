...
function ExperienceContainer({ experienceObjects, isDarkMode }) {
    return (
        <>
                ...
                <div key={index} className='flex flex-col md:flex-row w-full px-4 md:px-2'>
                    ...
                    <div className='w-full md:w-[65%] flex pt-3 md:pt-5'>
                        <div className='flex flex-col gap-y-1 sm:gap-y-2 md:gap-y-3 justify-start md:ml-[6%] w-full'>
                            <div className={`font-semibold text-xl sm:text-2xl md:text-3xl lg:text-3xl xl:text-4xl tracking-wide md:tracking-wider break-words md:break-normal ${isDarkMode ? 'text-gray-800' : ''}`}>
                                {experiences.Name}
                            </div>
                            <div className={`text-base sm:text-lg md:text-xl lg:text-2xl xl:text-2xl opacity-70 ${isDarkMode ? 'text-gray-800' : ''}`}>
                                {experiences.Des}
                            </div>
                        </div>
                    </div>
...
function Container({ type, Icon, objects, isDarkMode }) {
    return (
        <div className="w-full sm:w-[95%] md:w-[90%] lg:w-[650px] xl:w-[800px] min-h-[410px] sm:min-h-[460px] md:h-[510px] lg:h-[560px] xl:h-[610px] bg-gray-200 rounded-xl md:rounded-3xl flex flex-col gap-y-3 sm:gap-y-4 md:gap-y-5 items-center mx-4 sm:mx-auto md:mx-2 mb-8 md:mb-0">
            ...
            <div className='w-full flex flex-col gap-y-4 sm:gap-y-5 md:gap-y-6 lg:gap-y-8 xl:gap-y-9 pb-4 md:pb-0 overflow-y-auto'>
                <ExperienceContainer experienceObjects={objects} isDarkMode={isDarkMode} />
            </div>
        </div>
    );
}
       ...
       <div className="w-full bg-white pb-12 sm:pb-16 md:pb-20 lg:pb-24 xl:pb-32 pt-6 sm:pt-8 md:pt-10 lg:pt-12 xl:pt-14">
            <div className="flex flex-col">
                <div className="w-full flex flex-col gap-y-2 sm:gap-y-3 md:gap-y-4 items-center">
                    <div className={`font-sans text-xl sm:text-2xl md:text-2xl lg:text-3xl flex relative font-medium tracking-wide ${isDarkMode ? 'text-gray-800' : 'text-black'}`}>
                        <span>-</span>
                    ...
                    <div className={`flex flex-col md:flex-row text-center md:text-left relative text-2xl sm:text-3xl md:text-4xl lg:text-6xl xl:text-7xl gap-y-2 md:gap-x-3 lg:gap-x-4 xl:gap-x-5 tracking-wide md:tracking-wider -mt-8 sm:-mt-12 md:-mt-16 lg:-mt-20 xl:-mt-24 px-4 md:px-0 ${isDarkMode ? 'text-gray-800' : ''}`}>
                        <span>My</span>
                ...
                <div className="flex flex-col md:flex-row items-center justify-around w-full mt-6 sm:mt-8 md:mt-10 lg:mt-12 xl:mt-14 gap-8 md:gap-4">
                    <Container type="Education" Icon={SchoolIcon} objects={dataEducation} isDarkMode={isDarkMode} />
                    <Container type="Work Experience" Icon={WorkIcon} objects={dataWork} isDarkMode={isDarkMode} />
                </div>
            </div>
        </div>
    );
